...
trait RelationshipCompositionComponent {
  ...
  /** Composes a base relationship from an owner entity to a joining entity,
    * with an attached relationship from the joining entity to a target
    * relation, into a relationship from the owner entity to the target
    * relation.
    *
    ...
    */
  trait ComposedRelationship[
      Owner <: EntityTable[O, I],
      Join <: EntityTable[J, _],
      To <: Table[T],
      O <: Entity[O, I],
      J <: Entity[J, _],
      T,
      ...
      C1[_],
      C2[_]
  ] extends Relationship[Owner, To, O, I, T, C] {

    val baseRelationship: Relationship[Owner, Join, O, I, J, C1]
    ...
    ): Query[(Owner, To), (O, T), Seq] =
      attachedRelationship.expandJoin(baseRelationship.innerJoinFor(query))

    def expandJoin[W <: Table[E], E](
        from: Query[(W, Owner), (E, O), Seq]
    ): Query[(W, To), (E, T), Seq] =
  ...
  class ToManyComposed[
      ...
      val baseRelationship: Relationship[Owner, Join, O, I, J, Seq],
      val attachedRelationship: Relationship[Join, To, J, _, T, C]
  ) extends ComposedRelationship[Owner, Join, To, O, J, T, I, Seq, Seq, C]
      with ToManyRelationship[Owner, To, O, I, T]

  /** A relationship composed with a 'to many' relationship, which acts as a
    * 'to many' relationship.
    */
  class ComposedToMany[
      Owner <: EntityTable[O, I],
      Join <: EntityTable[J, _],
      To <: Table[T],
      O <: Entity[O, I],
  ...
  class ToOneComposedToOne[
      ...
      val baseRelationship: Relationship[Owner, Join, O, I, J, Option],
      val attachedRelationship: Relationship[Join, To, J, _, T, Option]
  ) extends ComposedRelationship[Owner, Join, To, O, J, T, I, Option, Option, Option]
      with ToOneRelationship[Owner, To, O, I, T]
  ...
  /** Implicitly adds the compose operation to 'to one' relationships whose
    ...
    * @tparam I   The relationship owner's ID type.
    */
  implicit class ToOneComposable[
      From <: EntityTable[O, I],
      Join <: EntityTable[J, _],
      O <: Entity[O, I],
      J <: Entity[J, _],
      I
  ](relationship: Relationship[From, Join, O, I, J, Option]) {
    ...
    /** Returns a composed 'to one' relationship by attaching the given 'to one'
      ...
      */
    def compose[To <: Table[T], T](
        target: Relationship[Join, To, J, _, T, Option]
    ): ToOneComposedToOne[From, Join, To, O, J, T, I] =
      new ToOneComposedToOne[From, Join, To, O, J, T, I](relationship, target)
    ...
    /** Returns a composed 'to many' relationship by attaching the given
      ...
      */
    def compose[To <: Table[T], T](
        target: Relationship[Join, To, J, _, T, Seq]
    ): ComposedToMany[From, Join, To, O, J, T, I, Option] =
      new ComposedToMany[From, Join, To, O, J, T, I, Option](
        relationship,
        target
      )
  }
  ...
  /** Implicitly adds the compose operation to 'to many' relationships whose
    ...
    * @tparam I   The relationship owner's ID type.
    */
  implicit class ToManyComposable[
      From <: EntityTable[O, I],
      Join <: EntityTable[J, _],
      O <: Entity[O, I],
      J <: Entity[J, _],
      I
  ](relationship: Relationship[From, Join, O, I, J, Seq]) {
    ...
    /** Returns a composed 'to many' relationship by attaching the given
      ...
      */
    def compose[To <: Table[T], T, C[_]](
        target: Relationship[Join, To, J, _, T, C]
    ): ToManyComposed[From, Join, To, O, J, T, I, C] =
      new ToManyComposed[From, Join, To, O, J, T, I, C](relationship, target)
  }
}
